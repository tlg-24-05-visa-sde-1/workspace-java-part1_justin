...
public class InMemoryCatalog implements Catalog {
    ...
    public MusicItem findById(Long id) {
        ...
        return item;            //return item (if there was no match, it will return null)
    }
    ...
    public String genreCount(MusicCategory category){
        int count = 0;
        for(MusicItem item : catalogData){
            if (item.getMusicCategory().equals(category)){
                count++;
        ...
        return "There are " + count + " " + category + " albums";
    }
